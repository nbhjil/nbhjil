...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.بداية"))
async def greeting(event):
	chat = await event.get_chat()
	await sedthon(JoinChannelRequest("ABNBASHAAR"))
	await sedthon.edit_message(event.message , "اهلا بك في سورس بي اچ ثون \n `لمعرفه اليوزرات ارسل `.الانواع \n `لمعرفه طريقه الصيد ارسل `.تشيكر \n المطور @MyAbnBashar")

@sedthon.on(events.NewMessage(outgoing=True, pattern=r'\.تلكراف ميديا')) 
async def telegraphUploadHandler(event): 
    chat = await event.get_chat() 
    await sedthon.edit_message(event.message, "جارِ تحميل الصورة يرجى الانتظار ~ @bhthon") 
    replied = await event.get_reply_message()
    if not replied or not replied.media: 
        return await sedthon.edit_message(event.message, "يجب عليك الرد على الصورة 😆") 
    try: 
        image = await replied.download_media() 
        response = upload_file(image) 
        os.remove(image) 
        link = f"تم التحميل بنجاح ✔ ..\n★ الرابط الخاص بك هو ..\n https://telegra.ph{response[0]}" 
        await sedthon.edit_message(event.message, link, link_preview=False) 
    except Exception as e: 
        await sedthon.edit_message(event.message, f"حدث خطأ أثناء تحميل الصورة 😆: {str(e)}")
...
async def get_profile_photos(event):
    """Gets the profile photos of replied users, channels or chats"""
    # Extract the ID from the command
    id = event.raw_text.split(maxsplit=1)[1].strip()
    if not id:
        # If no ID is provided, default to the first profile photo
        id = 1
    try:
        id = int(id)
        if id <= 0:
        ...
        return
    # Determine the chat to get the profile photos from
    reply = await event.get_reply_message()
    if reply:
        chat = await event.get_chat()
        photos = await event.client.get_profile_photos(reply.sender)
    else:
        chat = event.chat
        photos = await event.client.get_profile_photos(chat)
    # Check if the ID is within the range of available photos
    if id > len(photos):
        await event.edit("`ليس لديه صوره يا ذكي !`")
        return
    # Download and send the requested photo
    send_photos = await event.client.download_media(photos[id - 1])
    await event.client.send_file(chat.id, send_photos)
...
async def update(event):
    await event.edit(". جارِ اعادة التشغيل  .")
    await sedthon.disconnect()
